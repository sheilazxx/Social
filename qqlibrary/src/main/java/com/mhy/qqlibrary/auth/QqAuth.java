...
//         mTencent = Tencent.createInstance(QqSocial.getAppId()/*getInfo()*/, mActivity);//authInfo"101807669"
        if (mTencent != null && !mTencent.isSessionValid()) {
            mTencent.login(mActivity, "all", baseUiListener, true);//true 没装手q就扫码
...
//            info.getUserInfo(baseUiListener);
    ...
    public void doAuth(boolean isQR) {
        ...
        if (mTencent != null && !mTencent.isSessionValid()) {
            if (isQR) {
                mActivity.getIntent().putExtra(AuthAgent.KEY_FORCE_QR_LOGIN, true);//强制扫码/web登陆
                mTencent.login(mActivity, "all", baseUiListener, true);
            }//true 没装手q就扫码
            else {
                mTencent.login(mActivity, "all", baseUiListener);
            }
        } else if (mTencent != null) {
//            UserInfo info = new UserInfo(mActivity, mTencent.getQQToken());
//            info.getUserInfo(baseUiListener);
    ...
    public void doOpenMiniApp(String miniAppId, String miniAppPath, String miniAppVersion) {
        ...
        if (TextUtils.isEmpty(miniAppId)) {
            Toast.makeText(mActivity, "mini_app_id_empty", Toast.LENGTH_LONG).show();
            return;
        }
        // 校验版本类型
        if (!MiniApp.OPEN_CONNECT_DEMO_MINI_APP_VERSIONS.contains(miniAppVersion)) {
            Toast.makeText(mActivity, "mini_app_version_wrong", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private void launchMiniApp(String miniAppId, String miniAppPath, String miniAppVersion) {
        ...
        ret = mTencent.startMiniApp(mActivity, miniAppId, miniAppPath, miniAppVersion);
...
//            setErrorCallBack(builder.toString());
            Toast.makeText(mActivity, builder.toString(), Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void doIm(int mChosenIMType, String qq) {
        ...
        if (mTencent!=null&&mTencent.isSessionValid()) {
            ...
            if (!TextUtils.isEmpty(qq)) {
                ...
                Toast.makeText(mActivity, "QQ号为空，请重新设置", Toast.LENGTH_LONG).show();
                return;
            }
        } else {
            // 根据产品的更改，登录自己完成授权
            Toast.makeText(mActivity, "请先登陆", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void jumpIMWithType(int type, String qq) {
        ...
        if (type == Constants.IM_AIO) {
            ret = mTencent.startIMAio(mActivity, qq, mActivity.getPackageName());
        } else if (type == Constants.IM_AUDIO_CHAT) {
            ret = mTencent.startIMAudio(mActivity, qq, mActivity.getPackageName());
        } else if (type == Constants.IM_VIDEO_CHAT) {
            ret = mTencent.startIMVideo(mActivity, qq, mActivity.getPackageName());
        }
